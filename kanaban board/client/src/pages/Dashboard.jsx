                ...
                >
                  ...
                  >
                    <span
                      ...
                      style={{ background: b.color || "#1a7fd4" }}
                    />
                    <div className="mb-3.5 flex items-start justify-between">
                      <div
                        className="flex h-11 w-11 items-center justify-center rounded-2xl"
                        style={{
                          backgroundColor: `${b.color || "#1a7fd4"}1f`,
                          color: b.color || "#1a7fd4",
                        }}
                      >
    ...
    >
      ...
      {hasTasks ? (
        <div className="mt-2 flex flex-1 flex-col gap-6 lg:flex-row">
          {/* Bars — each uses the board's own accent color */}
          <div className="flex h-[230px] flex-1 items-end gap-3">
            {boards.map((b, i) => {
              const count = Number(b.task_count || 0);
              const pct = count / max;
              const color = b.color || "#1a7fd4";
              // Darken the bar for taller columns (more contrast), lighten for shorter ones
              const barOpacity = Math.max(0.55, pct * 0.9 + 0.1);
              return (
                <Link
                  key={b.id}
                  to={`/board/${b.id}`}
                ...
                >
                  <div className="relative flex w-full max-w-[44px] flex-1 items-end justify-center">
                    {count > 0 ? (
                      <>
                        {/* Hover tooltip */}
                        <span className="absolute left-1/2 top-0 z-10 -translate-x-1/2 -translate-y-6 whitespace-nowrap rounded-full px-2 py-0.5 text-[10px] font-semibold tabular text-white opacity-0 shadow-[var(--shadow-soft)] transition-all duration-200 group-hover:opacity-100 group-hover:-translate-y-7"
                          style={{ backgroundColor: color }}
                        >
                          {count} tasks
                        </span>
                        {/* Animated bar */}
                        <motion.div
                          className="w-full rounded-full transition-shadow duration-200"
                          style={{
                  ...
                  <span
                    className="grid h-7 w-7 shrink-0 place-items-center rounded-lg font-display text-[11px] font-bold transition-transform duration-200 group-hover:scale-110"
              ...
              {boards.map((b) => {
                ...
                return (
                  ...
                  >
                    <span
                      className="h-2.5 w-2.5 shrink-0 rounded-full"
                      style={{ backgroundColor: b.color || "#1a7fd4" }}
                    />
...
// ── Workspace composition (per-board colored donut) ──────────────────────
const WorkspaceDonut = ({ owned, shared, boards = [], className }) => {
  ...
  // Build per-board donut segments (equal slices by board count, or by task_count)
  const segments = boards.map((b, i) => ({
    color: b.color || "#1a7fd4",
    title: b.title,
    count: Number(b.task_count || 0),
    share: totalTasks > 0 ? (Number(b.task_count || 0) / totalTasks) : (1 / boards.length),
  }));

  // Calculate stroke-dasharray for each segment
  let offset = 0;
  const segmentData = segments.map(seg => {
    const len = seg.share * C - GAP;
  ...
  return (
    ...
    >
      <SectionTitle icon={PieChart} hint="Task distribution by board">
        Composition
      </SectionTitle>
      <div className="flex items-center gap-5">
        <div className="relative h-[128px] w-[128px] shrink-0">
          <svg viewBox="0 0 140 140" className="h-full w-full -rotate-90">
            {/* Track */}
            <circle cx="70" cy="70" r={R} fill="none" stroke="var(--color-surface-2)" strokeWidth={SW} />
            {/* Per-board segments */}
            {total > 0 && segmentData.map((seg, i) => (
              <circle
                key={i}
                cx="70" cy="70" r={R}
                fill="none"
        ...
        {/* Per-board legend (top 4) */}
        <div className="flex flex-1 flex-col gap-2.5 min-w-0">
          {boards.slice(0, 4).map((b) => (
            <div key={b.id} className="flex items-center gap-2 min-w-0">
              <span className="h-2.5 w-2.5 shrink-0 rounded-full" style={{ backgroundColor: b.color || "#1a7fd4" }} />
              <span className="flex-1 truncate text-[12px] text-muted">{b.title}</span>
              <span className="text-[12px] font-semibold tabular text-ink shrink-0">{b.task_count ?? 0}</span>
            </div>
          ))}
          {boards.length > 4 && (
            <p className="text-[11px] text-faint">+{boards.length - 4} more</p>
          )}
        </div>
      </div>
