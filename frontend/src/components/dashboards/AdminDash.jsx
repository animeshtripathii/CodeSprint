...
import {
  Shield, Users, Trophy, FileText, BarChart2,
  RefreshCw, Search, UserX, UserCheck, Trash2,
  Activity, Clock, Zap, AlertTriangle, Eye,
...
import {
  ...
  toggleUserBlock, deleteUser, fetchAllHackathons,
  updateHackathonStatus, deleteHackathon,
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        {subtitle && <div style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.35)', marginTop: 2 }}>{subtitle}</div>}
      </div>
      {trend !== undefined && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: '0.72rem', color: trend >= 0 ? '#34d399' : '#f87171', fontWeight: 600 }}>
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
      <div>
        ...
        {subtitle && <p style={{ fontSize: '0.78rem', color: 'rgba(255,255,255,0.45)', margin: '4px 0 0 0' }}>{subtitle}</p>}
      </div>
      {action}
    </div>
  );
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: 20 }}>
          ...
          {recentUsers.length === 0 ? (
            <div style={{ color: 'rgba(255,255,255,0.3)', fontSize: '0.8rem', textAlign: 'center', padding: '16px 0' }}>No users found</div>
          ) : (
              ...
              {recentUsers.slice(0, 6).map(u => (
                <div key={u._id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: '0.8rem', fontWeight: 600, color: '#fff', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{u.name}</div>
                    <div style={{ fontSize: '0.68rem', color: 'rgba(255,255,255,0.4)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{u.email}</div>
                  </div>
        ...
        <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: 20 }}>
          ...
          {recentHackathons.length === 0 ? (
            <div style={{ color: 'rgba(255,255,255,0.3)', fontSize: '0.8rem', textAlign: 'center', padding: '16px 0' }}>No hackathons yet</div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {recentHackathons.slice(0, 6).map(h => (
                <div key={h._id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: '0.8rem', fontWeight: 600, color: '#fff', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{h.title}</div>
                    <div style={{ fontSize: '0.68rem', color: 'rgba(255,255,255,0.4)' }}>{h.registrations || 0} registered</div>
                  </div>
...
function UsersTab({ users, onRoleChange, onToggleBlock, onDelete }) {
  ...
  return (
    <div>
      {confirmDelete && (
        <ConfirmModal
          message="This will permanently delete the user and all their associated data. This cannot be undone."
          onConfirm={handleConfirmDelete}
          onCancel={() => setConfirmDelete(null)}
        />
      )}

      <SectionHeader title="User Management" subtitle={`${filtered.length} user${filtered.length !== 1 ? 's' : ''} shown`} />
            ...
            <tbody>
              {filtered.map((u, idx) => (
                ...
                >
                  ...
                  <td style={{ padding: '12px 16px', fontSize: '0.8rem', color: 'rgba(255,255,255,0.55)' }}>{u.email}</td>
                  <td style={{ padding: '12px 16px' }}>
                    <select
                      value={u.role}
                  ...
                  <td style={{ padding: '12px 16px', textAlign: 'right' }}>
                    <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
                      <ActionBtn onClick={() => onToggleBlock(u._id, u.blocked)} title={u.blocked ? 'Unblock user' : 'Block user'} danger={!u.blocked}>
                        {u.blocked ? <UserCheck size={14} /> : <UserX size={14} />}
                      </ActionBtn>
                      <ActionBtn onClick={() => setConfirmDelete(u._id)} title="Delete user" danger>
                        <Trash2 size={14} />
                      </ActionBtn>
                    </div>
                  </td>
...
function HackathonsTab({ hackathons, onStatusChange, onDelete, onView }) {
  ...
  return (
    <div>
      {confirmDelete && (
        <ConfirmModal
          message="This will permanently delete the hackathon along with all registrations, teams, and submissions."
          onConfirm={handleConfirmDelete}
          onCancel={() => setConfirmDelete(null)}
        />
      )}

      <SectionHeader title="Hackathon Oversight" subtitle={`${filtered.length} hackathon${filtered.length !== 1 ? 's' : ''} shown`} />
...
function AuditLogTab({ users, hackathons }) {
  // Build synthetic events from loaded data since the audit endpoint may not exist yet
  const events = [
  ...
  return (
    <div>
      <SectionHeader title="Platform Audit Log" subtitle="Recent activity across all users, hackathons, and system events" />
      <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: 24 }}>
        {events.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {events.map((evt, idx) => (
              <div key={evt.id} style={{ display: 'flex', gap: 14, alignItems: 'flex-start', paddingBottom: idx < events.length - 1 ? 18 : 0, position: 'relative' }}>
                {/* Connecting line between events */}
                {idx < events.length - 1 && (
                  <div style={{ position: 'absolute', left: 14, top: 30, bottom: 0, width: 1, background: 'rgba(255,255,255,0.06)' }} />
                )}
                <div style={{
                ...
                <div style={{ paddingTop: 4, flex: 1 }}>
                  <div style={{ fontSize: '0.82rem', color: 'rgba(255,255,255,0.8)', lineHeight: 1.4 }}>{evt.message}</div>
                  <div style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.35)', marginTop: 3 }}>{formatTime(evt.time)}</div>
                </div>
              </div>
            ))}
          </div>
...
export default function AdminDash() {
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', background: '#050507', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16 }}>
        <div style={{ display: 'flex', gap: 6 }}>
          {[0, 1, 2].map(i => (
            <div key={i} style={{ width: 8, height: 8, borderRadius: '50%', background: '#a78bfa', animation: `pulse 0.9s ${i * 0.15}s ease-in-out infinite alternate` }} />
          ))}
        </div>
        <div style={{ fontSize: '0.85rem', color: 'rgba(255,255,255,0.4)' }}>Loading admin console...</div>
  ...
  return (
    <div style={{ position: 'relative', background: '#050507', minHeight: '100vh', color: '#fff', overflow: 'hidden' }}>
      <DottedGlowBackground gap={20} radius={1.5} opacity={0.5} color="rgba(167,139,250,0.2)" glowColor="rgba(167,139,250,0.4)" />
      ...
      <div style={{ position: 'relative', zIndex: 10, maxWidth: 1200, margin: '0 auto', padding: '32px 28px' }}>
        ...
        <div style={{ marginBottom: 32 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 8 }}>
            ...
            <div>
              <h1 style={{ fontFamily: "'Instrument Serif', serif", fontSize: '2.2rem', fontWeight: 700, color: '#fff', margin: 0, lineHeight: 1.1 }}>
                Admin Control
              </h1>
        ...
        <div style={{ display: 'flex', gap: 4, marginBottom: 28, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: 4 }}>
          {tabs.map(tab => (
            <button
              ...
              onMouseEnter={e => {
                if (activeTab !== tab.id) { e.currentTarget.style.background = 'rgba(255,255,255,0.05)'; e.currentTarget.style.color = 'rgba(255,255,255,0.75)'; }
              }}
              onMouseLeave={e => {
                if (activeTab !== tab.id) { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'rgba(255,255,255,0.5)'; }
              }}
            >
              {tab.icon}
              {tab.label}
        ...
        {/* Tab content */}
        {activeTab === 'overview' && (
          <OverviewTab stats={stats} recentUsers={users} recentHackathons={hackathons} onRefresh={loadAllData} refreshing={refreshing} />
        )}
        {activeTab === 'users' && (
          <UsersTab users={users} onRoleChange={handleRoleChange} onToggleBlock={handleToggleBlock} onDelete={handleDeleteUser} />
        )}
        {activeTab === 'hackathons' && (
          <HackathonsTab hackathons={hackathons} onStatusChange={handleStatusChange} onDelete={handleDeleteHackathon} onView={id => navigate(`/hackathons/${id}`)} />
        )}
        {activeTab === 'audit' && (
